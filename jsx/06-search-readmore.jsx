...
// ─── Search Screen ──────────────────────────────────────────────────
function SearchScreen({ initialQuery = '', onBack, onPick, contentType = 'both' }) {
  const [query, setQuery] = React.useState(initialQuery);
  const inputRef = React.useRef(null);
  React.useEffect(()=>{ inputRef.current?.focus(); }, []);

  // Filter mock catalog
  const TRENDING_IDS = ['m1','m2','m9','m11','m14','m6'];
  const results = !query.trim()
    ? (window.MOVIES || []).filter(m => TRENDING_IDS.includes(m.id))
    ...
    <div className="fade-in" style={{display:'flex', flexDirection:'column', height:'100%'}}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <input
            ref={inputRef}
            placeholder={contentType==='series' ? 'Search series…' : contentType==='movies' ? 'Search films…' : 'Search films & series…'}
            value={query}
            onChange={e=>setQuery(e.target.value)}
            style={{
              flex:1, background:'transparent', border:0, outline:0,
              color:'var(--cream)', fontFamily:'var(--sans)', fontSize: 14.5, letterSpacing:'-0.01em',
            }}
          />
          {query && (
      ...
      <div style={{padding:'4px 16px 14px', display:'flex', gap: 6}}>
        {['All', 'Movies', 'Series'].map((t, i) => (
          <button key={t} style={{
            appearance:'none', border:`0.5px solid ${i===0 ? 'var(--cream)':'rgba(var(--fg-rgb),0.12)'}`,
            background: i===0 ? 'rgba(var(--fg-rgb),0.08)' : 'transparent',
      ...
      {/* Results / Trending */}
      <div className="phone-scroll" style={{flex:1, overflowY:'auto', padding:'0 16px 120px'}}>
        {!query && (
          <div style={{
        ...
        {query && (
          <div style={{
            fontSize: 13, color:'var(--muted)', padding:'4px 4px 12px',
          }}>
            {results.length} result{results.length===1?'':'s'} for "<span style={{color:'var(--cream)'}}>{query}</span>"
          </div>
        )}
        ...
        <div style={{display:'flex', flexDirection:'column', gap: 10}}>
          {results.map(m=>(
            <button key={m.id} onClick={()=>onPick(m)} style={{
              appearance:'none', border:0, background:'rgba(var(--fg-rgb),0.03)',
              textAlign:'left',
              ...
              <div style={{width: 60, height: 90, borderRadius: 8, overflow:'hidden', flexShrink:0}}>
                <Poster movie={m} size="sm" hideTitle/>
              </div>
              <div style={{flex: 1, minWidth: 0}}>
                ...
                <div style={{
                  ...
                  {m.year} · {m.genres?.[0]} · {m.rt}% RT
                </div>
                <div style={{display:'flex', gap: 4, marginTop: 8}}>
                  {m.where?.slice(0,3).map(s => <ServiceChip key={s} name={s} size={18}/>)}
                </div>
              </div>
        ...
        {query && results.length === 0 && (
          <div style={{
            textAlign:'center', padding:'40px 20px', color:'var(--muted)', fontSize: 13,
          }}>
            No results. Try a different title or genre.
          </div>
        )}
      </div>
    </div>
...
Object.assign(window, { ReadMoreSheet, SearchScreen, SearchOverlay });
